...
// de eerste procenten van de baan geperst worden, precies waar je met een vinger
// op een kuipscherm het minst nauwkeurig bent. Daarom schuift hij langs deze
// vaste reeks: fijn waar het om scheepslengtes gaat, grof waar het om uren gaat.
export const STRAAL_NM = [0.1, 0.2, 0.3, 0.5, 0.75, 1, 1.5, 2, 3, 4,
                          5, 7.5, 10, 15, 20, 25, 30, 35, 40, 45];

// Bij welke stap hoort een bewaarde waarde? De dichtstbijzijnde, zodat een
// waarde van voor deze reeks (of met de hand gezet) blijft staan waar hij hoort
// in plaats van naar het begin te springen.
export function straalIndex(nm, schaal = STRAAL_NM) {
  const v = Number(nm);
  if (!Number.isFinite(v)) return 0;
  let beste = 0;
  for (let i = 1; i < schaal.length; i++) {
    if (Math.abs(schaal[i] - v) < Math.abs(schaal[beste] - v)) beste = i;
  }
  return beste;
}

// En terug: welke straal hoort bij stap i. Buiten de reeks wordt afgekapt, zodat
// een rare schuifstand nooit een onbedoelde straal oplevert.
export function straalWaarde(i, schaal = STRAAL_NM) {
  const n = Math.round(Number(i));
  if (!Number.isFinite(n)) return schaal[0];
  return schaal[Math.max(0, Math.min(schaal.length - 1, n))];
}
if (typeof window !== 'undefined') {
  window.STRAAL_NM = STRAAL_NM;
  window.straalIndex = straalIndex; window.straalWaarde = straalWaarde;
}
